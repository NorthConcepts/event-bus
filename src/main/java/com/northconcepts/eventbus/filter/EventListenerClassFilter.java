...
public class EventListenerClassFilter implements EventFilter {
	
	private final Class<?>[] eventListenerClass;

	public EventListenerClassFilter(Class<?> ... eventListenerClass) {
		this.eventListenerClass = eventListenerClass;
	}

	@Override
	public boolean allow(Event<?> event, Object listener) {
		if (eventListenerClass == null || eventListenerClass.length == 0) {
			return true;
		}
		for (Class<?> c : eventListenerClass) {
			if (event.getEventListenerClass() == c) {
				return true;
			}
		}
		return false;
